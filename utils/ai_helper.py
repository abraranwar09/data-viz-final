...
def get_ai_insights(question, data):
    ...
    try:
        ...
        context = {
            'question': question or "Analyze this data and create the most insightful visualization",
            'data': data.get('preview', [])[:5],  # First 5 rows for context
            'columns': list(data.get('preview', [{}])[0].keys()) if data.get('preview') else [],
            'column_stats': data.get('column_stats', {})  # Add statistics if available
        }
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": """You are an expert in data visualization using ECharts. Create a visualization that best answers the user's request or reveals key insights in the data. Your response must be a valid ECharts configuration matching this schema:
